...
contract KRWTCustodianWithOracleTest is Test {
    ...
    function testDepositMintWithdrawRedeem_UpdateOracle() public {
        ...
        vm.prank(owner);
        custodian.setWhitelistStatus(user, true);
    ...
    function testWhitelist_AddToWhitelist() public {
        vm.prank(owner);
        custodian.setWhitelistStatus(whitelistedUser, true);
    ...
    function testWhitelist_RemoveFromWhitelist() public {
        ...
        vm.prank(owner);
        custodian.setWhitelistStatus(whitelistedUser, false);
    ...
    function testWhitelist_DepositWithdrawNotRestricted() public {
        // Set contract to public mode so deposit and withdraw are not restricted by whitelist
        vm.prank(owner);
        custodian.setPublic(true);
        ...
        // Deposit should work even when not whitelisted (because public mode is enabled)
        ...
        // Withdraw should work even when not whitelisted (because public mode is enabled)
        krwt.approve(address(custodian), type(uint256).max);
